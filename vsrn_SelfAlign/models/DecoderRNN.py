...
class DecoderRNN(nn.Module):
    ...
    def forward(self,
        ...
        - **targets** (batch, max_length): targets labels of the ground truth sentences,configure max_length=60,add 'end' to 61
        ...
        batch_size, _, _ = encoder_outputs.size() # [bt,36,emb_size]
        decoder_hidden = self._init_rnn_state(encoder_hidden) # decoder_hidden = encoder_hidden,because of it not bidirectional rnn

        seq_logprobs = []
        seq_preds = []
        self.rnn.flatten_parameters() # to improve the efficiency,make the params contiguous
        if mode == 'train':
            # use targets as rnn inputs
            # print(targets)
            targets_emb = self.embedding(targets) # [bt,max_len,300]
            for i in range(self.max_length - 1):
                current_words = targets_emb[:, i, :]
                context = self.attention(decoder_hidden.squeeze(0), encoder_outputs) # [bt,2048]
                decoder_input = torch.cat([current_words, context], dim=1) # [bt,300+2048]
                decoder_input = self.input_dropout(decoder_input).unsqueeze(1) # [bt,1,2348]
                decoder_output, decoder_hidden = self.rnn(
                    decoder_input, decoder_hidden) # [bt,1,2048] [1,128,2048]
                logprobs = F.log_softmax(
                    self.out(decoder_output.squeeze(1)), dim=1) # [bt,vovab_size=8481]
