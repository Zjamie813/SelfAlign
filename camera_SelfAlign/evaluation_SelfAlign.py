...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        if no_init:
            ...
            img_ctx_feas = np.zeros((len(data_loader.dataset), img_ctx_fea.size(1)), dtype=np.float32)
            cap_ctx_feas = np.zeros((len(data_loader.dataset), cap_ctx_fea.size(1)), dtype=np.float32)

        # preserve the embeddings by copying from gpu and converting to numpy
        img[ids] = img_emb.detach().cpu().numpy().copy()
        cap[ids] = cap_emb.data.cpu().numpy().copy()
        img_ctx_feas[ids] = img_ctx_fea.detach().cpu().numpy().copy()
        cap_ctx_feas[ids] = cap_ctx_fea.detach().cpu().numpy().copy()
...
def i2t(sims, npts=None, return_ranks=False):
    ...
    for index in range(npts):
        ...
        ranks[index] = rank
        top1[index] = inds[0]
    print('camera rank time:%.2fs' % (time.time() - t0))
...
def t2i(sims, npts=None, return_ranks=False):
    ...
    print('rank  time:%.2fs' % (time.time() - to))
    # Compute metrics
    r1 = 100.0 * len(np.where(ranks < 1)[0]) / len(ranks)
    r5 = 100.0 * len(np.where(ranks < 5)[0]) / len(ranks)
    r10 = 100.0 * len(np.where(ranks < 10)[0]) / len(ranks)
