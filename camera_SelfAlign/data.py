...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, opt):
        ...
        with open(loc+'%s_ims.npy' % data_split, 'r') as images_file:
            with open(loc+'%s_boxes.npy' % data_split,'r') as box_file:
                with open(loc+'%s_img_sizes.npy' % data_split,'r') as wh_file:
                    self.images = np.load(images_file)
                    self.boxes = np.load(box_file)
                    self.img_wh = np.load(wh_file)  # (num_img, 2)
