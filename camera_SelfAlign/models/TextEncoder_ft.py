...
from camera_SelfAlign.bert import BertModel, BertConfig
...
class TextEncoder(nn.Module):
    ...
    def __init__(self, cfg_file, init_ckpt, embed_size, head, drop=0.0):
        ...
        ckpt_bert = OrderedDict()  # weight---beta,bias--gamma,they are single dimension vector[768]
        for k,v in ckpt.items():
            prefix = k.split('.')
            if prefix[0] == 'bert':
                prefix = prefix[1:]
            # if prefix[-2] == 'LayerNorm':
            #     if prefix[-1] == 'weight':
            #         prefix[-1] = 'beta'
            #     if prefix[-1] == 'bias':
            #         prefix[-1] = 'gamma'
            new_k = '.'.join(prefix)
            ckpt_bert[new_k]=v
        # self.bert.load_state_dict(ckpt_bert,strict=False) # 多了unexped keys
        self.bert.load_state_dict(ckpt_bert,strict=False)
